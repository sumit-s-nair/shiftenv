...
def grade(repo_dir: str | None = None, trajectory: dict | None = None) -> float:
    """Simple medium-task grader with strict (0, 1) score bounds."""
    ...
    files = ["api_client.py", "session_manager.py", "downloader.py"]

    checks: list[bool] = []
    for rel_path in files:
        fpath = os.path.join(repo_dir, rel_path)
        imports = _get_imports(fpath)
        checks.append("requests" not in imports)
        checks.append("httpx" in imports)
